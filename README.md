...
`source_url` показывается в сообщениях модерации, чтобы администратор мог проверить контекст. В финальный текст поста строка `Источник:` автоматически не добавляется и при необходимости удаляется перед сохранением.
