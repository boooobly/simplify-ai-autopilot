...
## Recovery and failed drafts

- Перед публикацией запланированный черновик атомарно переводится из `scheduled` в `publishing` (lock), чтобы один и тот же пост не ушёл дважды при параллельных тиках планировщика.
- Черновики со статусом `failed` не ретраятся автоматически.
- Для ручной диагностики используй `/failed_drafts` (последние 10 failed).
- Для ручного восстановления используй `/restore_draft ID` (только для статуса `failed`, возвращает в `draft` и очищает `scheduled_at`).
- При рестарте бота все зависшие `publishing` автоматически переводятся в `failed` для безопасной ручной проверки.
